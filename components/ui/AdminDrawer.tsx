...
const MENU_ITEMS = [
  { section: 'GESTION',        items: [
    ...
    { icon: '🗓',  label: 'Seances', path: '/admin/calendar' },
    { icon: '📈', label: 'Stats',       path: '/admin/stats' },
  ]},
  { section: 'ADMINISTRATION', items: [
    { icon: '🏅', label: 'Coaches',     path: '/admin/coaches' },
    { icon: '👥', label: 'Membres',     path: '/admin/members' },
    { icon: '💳', label: 'Abonnements', path: '/admin/abonnements' },
  ]},
  { section: 'Presence', items: [
  { 
    icon: '📷', 
    label: 'Scan Code',     
    path: '/admin/scan' 
          ...
          {user && (
            <View style={styles.userRow}>
              <View style={styles.userAvatar}>
                <Text style={styles.userAvatarTxt}>{user.nom.charAt(0).toUpperCase()}</Text>
              </View>
              <View>
...
const styles = StyleSheet.create({
  ...
  userAvatar:    { width: 40, height: 40, borderRadius: 20, backgroundColor: Colors.blueDim, borderWidth: 1, borderColor: Colors.blue, alignItems: 'center', justifyContent: 'center' },
  userAvatarTxt: { color: Colors.blue, fontWeight: '800', fontSize: 18 },
