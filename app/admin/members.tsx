...
export default function AdminMembersScreen() {
  ...
  const subMut = useMutation({
    mutationFn: (body: any) => api.post('/abonnements', body).then(r => r.data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['members'] }); setShowSub(null); resetSub(); Alert.alert('✅ Abonnement créé !') },
    onError: (e: any) => Alert.alert('Erreur', e.response?.data?.detail ?? 'Erreur'),
  })

  const resetCreate = () => { setNom(''); setEmail(''); setPass(''); setTel('') }
  const resetSub    = () => { setSubType('basic'); setDateDebut(''); setDateFin('') }
  ...
  const confirmDelete = (m: Member) =>
    Alert.alert(`Supprimer ${m.nom} ?`, 'Irréversible — toutes ses réservations seront supprimées.', [
    ...
    <Card style={[styles.card, m.is_flagged && { borderColor: Colors.warning + '50' }]} elevated>
      ...
      {m.is_flagged && (
        <View style={styles.flagRow}>
          <Text style={styles.flagTxt}>⚠️ {m.no_show_count} absence{m.no_show_count > 1 ? 's' : ''} non justifiée{m.no_show_count > 1 ? 's' : ''}</Text>
        </View>
      )}
      ...
      <View style={styles.memberActions}>
        ...
        <TouchableOpacity style={[styles.aBtn, { borderColor: Colors.gold }]} onPress={() => setShowSub(m.id)}>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.black }}>
      ...
      <Modal visible={showSub !== null} animationType="slide" transparent onRequestClose={() => setShowSub(null)}>
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            <View style={styles.handle} />
            <Text style={styles.sheetTitle}>Abonnement — Membre #{showSub}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.sheetActions}>
                <Button title="Annuler" variant="ghost" style={{ flex: 1 }} onPress={() => { setShowSub(null); resetSub() }} />
                <Button title="Créer" variant="gold" loading={subMut.isPending} style={{ flex: 1 }}
                  onPress={() => {
                    if (!dateDebut || !dateFin) return Alert.alert('Requis', 'Les dates sont obligatoires. Format: YYYY-MM-DD')
                    subMut.mutate({ utilisateur_id: showSub, type: subType, date_debut: dateDebut, date_fin: dateFin })
                  }} />
              </View>
            </ScrollView>
          </View>
